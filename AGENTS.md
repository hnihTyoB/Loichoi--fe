...
- ☁️ **Xanh pastel / Xanh baby (`#CDE4FE` / `#A2CFFE`)**: Màu của bầu trời và đôi mắt Cinnamoroll, dùng cho các nút bấm hành động chính (CTA), đường viền hoặc vùng highlight.
...
### ⚡ 1.4. Hiệu ứng chuyển động (Micro-interactions & Animation)
- **Hiệu ứng bồng bềnh (Floating Animation)**: Các icon đám mây, sticker, hình minh họa chuyển động lên xuống nhẹ nhàng nhờ CSS Animation / Framer Motion.
...
- **Con trỏ chuột tùy biến (Custom Cursor)**: Hình đám mây nhỏ hoặc dấu chân cún con.

### 🖼️ 1.5. Bố cục & Hình ảnh minh họa (Layout & Illustrations)
...
- **Hình vẽ Vector / 3D Clay**: Sử dụng hình minh họa 2D dạng lofi nét liền hoặc các khối 3D mềm mại như đất sét nặn (Claymorphism).

### 🛠️ 1.6. Ý tưởng bố cục chuẩn cho các màn hình
| Phần (Section) | Ý tưởng thiết kế Kawaii Cinnamoroll |
| :--- | :--- |
| **Header** | Thanh menu màu trắng kem bo góc lớn, logo tai cún mềm, nút đổi màu xanh pastel khi hover. |
| **Hero Section** | Tiêu đề lớn màu nâu mocha ấm, nút bấm màu xanh baby bồng bềnh, họa tiết đám mây/bong bóng. |
...
| **Styling** | **Tailwind CSS** | Bảng màu Cinnamoroll Pastel + HSL CSS variables, Dark Mode class-based. |
| **UI Components** | **shadcn/ui + Radix UI** | Primitives tại `src/components/ui/` với góc bo `rounded-2xl`/`rounded-3xl` và bóng mây. |
| **Icon System** | **Lucide React** | Icons nét bo tròn, phối màu pastel dịu mắt. |
...
| **State Global** | **Zustand** | Chỉ dùng cho Client State thực sự cần thiết (Auth, UI toggles) tại `src/stores/`. |
...
└── src/                                      # Thư mục mã nguồn ứng dụng
    ...
    │   ├── shared/                           # Components tái sử dụng (Header, Sidebar, PermissionGate...)
    ...
    ├── lib/                                  # Utilities & configurations (utils.ts, api-client.ts...)
...
1. **Tuân Thủ Phong Cách Cute Kawaii (Cinnamoroll Style)**:
...
2. **Phân biệt Server Component và Client Component**:
