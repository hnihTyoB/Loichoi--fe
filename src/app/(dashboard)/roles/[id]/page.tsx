...
export default function RoleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (role.data) {
      setSelected(role.data.permissions?.map((item) => item.id) ?? []);
  ...
  const grouped = useMemo(
    () =>
      (permissionData ?? []).reduce<Record<string, NonNullable<typeof permissionData>>>((result, permission) => {
  ...
  const saveMetadata = useMutation({
    mutationFn: () => rbacService.updateRole(id, { name, description }),
    onSuccess: () => {
      toast.success(isMounted ? t.adminRoles.updateRoleSuccess : "Đã cập nhật vai trò");
      client.invalidateQueries({ queryKey: ["roles"] });
    },
    onError: (error) => toast.error(getErrorMessage(error)),
  });
    ...
    <PermissionGate permission={PERMISSIONS.ROLE_READ} fallback={<AsyncState error />}>
      <div className="space-y-6">
        <Button asChild variant="ghost" size="sm">
          <Link href="/roles">
            <ArrowLeft />
            {isMounted ? t.adminRoles.backToRoles : "Quay lại"}
          </Link>
        </Button>
        <PageHeader
          icon={ShieldCheck}
          title={`${isMounted ? t.adminRoles.matrixTitlePrefix : "Ma trận quyền:"} ${role.data.name}`}
          description={isMounted ? t.adminRoles.matrixDesc : "Tích chọn quyền theo nhóm tài nguyên và lưu đồng bộ."}
          actions={
            <PermissionGate permission={PERMISSIONS.ROLE_PERMISSION_ASSIGN}>
              <Button disabled={savePermissions.isPending} onClick={() => savePermissions.mutate()}>
                ...
                {savePermissions.isPending
                  ...
                  : (isMounted ? t.adminRoles.saveMatrixBtn : "Lưu ma trận")}
              </Button>
            </PermissionGate>
          }
        />
        {!role.data.isSystem && (
          <PermissionGate permission={PERMISSIONS.ROLE_UPDATE}>
            <Card>
              <CardContent className="grid gap-4 pt-6 md:grid-cols-[220px_1fr_auto] md:items-end md:pt-8">
                <Field label={isMounted ? t.adminRoles.roleNameLabel : "Tên vai trò"}>
                  <Input value={name} onChange={(event) => setName(event.target.value.toUpperCase())} />
                </Field>
                <Field label={isMounted ? t.adminRoles.roleDescLabel : "Mô tả"}>
                  <Textarea className="min-h-11" value={description} onChange={(event) => setDescription(event.target.value)} />
                </Field>
                <Button variant="outline" disabled={saveMetadata.isPending} onClick={() => saveMetadata.mutate()}>
                  {saveMetadata.isPending
                    ? (isMounted ? t.adminUi.saving : "Đang lưu...")
                    : (isMounted ? t.adminRoles.saveInfoBtn : "Lưu thông tin")}
                </Button>
              </CardContent>
            </Card>
          </PermissionGate>
            ...
            <Card key={resource}>
              <CardContent className="pt-6 md:pt-8">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="font-black text-kawaii-mocha">{resource}</h2>
                  <Badge variant="secondary">
                    {items?.filter((item) => selected.includes(item.id)).length ?? 0}/{items?.length ?? 0}
                  </Badge>
                </div>
                        ...
                        onChange={(event) =>
                          setSelected(
                            event.target.checked
                              ...
                              : selected.filter((item) => item !== permission.id),
                          )
                        }
                      />
