...
export const notificationService = {
  ...
  async getEmails(params: { status?: string; toEmail?: string; page?: number; limit?: number } = {}): Promise<PageResult<EmailItem>> { const response = await apiClient.get<PageResult<EmailItem>>("/notifications/emails", { params }); return response.data; },
  async retryEmail(id: string): Promise<void> { await apiClient.post(`/notifications/emails/${id}/retry`); },
  async getTemplates(params: { search?: string; channel?: string; isActive?: boolean; page?: number; limit?: number } = {}): Promise<PageResult<NotificationTemplate>> { const response = await apiClient.get<PageResult<NotificationTemplate>>("/notifications/templates", { params }); return response.data; },
